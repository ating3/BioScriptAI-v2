...
import * as ProjectStorage from '../services/project-storage.js';
...
chrome.storage.local.get(['settings'], (result) => {
  if (result.settings) {
    ...
    if (settings.llmBaseUrl) llmService.baseUrl = settings.llmBaseUrl;
  }
});

/** Ensure LLM service uses current saved base URL before a call */
async function applyLLMSettings() {
  const result = await chrome.storage.local.get(['settings']);
  if (result.settings?.llmBaseUrl) {
    llmService.baseUrl = result.settings.llmBaseUrl;
  }
}
...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  ...
  if (request.action === 'projects') {
    handleProjectsRequest(request, sendResponse);
    return true;
  }
});
...
/**
 * Handle local projects (no Zotero required)
 */
async function handleProjectsRequest(request, sendResponse) {
  try {
    ...
    switch (operation) {
      case 'listProjects': {
        ...
        const withCount = projects.map(p => ({
          ...p,
          paperCount: (p.paperIds || []).length
        }));
        sendResponse({ success: true, projects: withCount });
        break;
      }

      case 'createProject': {
        const project = await ProjectStorage.createProject(params.name);
        sendResponse({ success: true, project });
        break;
      }

      case 'getProjectPapers': {
        const papers = await ProjectStorage.getProjectPapers(params.projectId);
        sendResponse({ success: true, papers });
        break;
      }

      case 'addPaperToProject': {
        const { projectId, paper, paperContext } = params;
        let summary = null;
        if (paperContext && paperContext.trim().length > 100) {
          await applyLLMSettings();
          try {
            const result = await summarizationModule.summarize({
              paperContext,
              summaryGoal: 'balanced',
              focusWeight: 'medium'
            });
            summary = result;
          } catch (e) {
            console.warn('Summary generation failed, saving paper without summary:', e);
          }
        }
        const { paperId, isNew } = await ProjectStorage.addPaperToProject(
          projectId,
          paper,
          paperContext,
          summary
        );
        sendResponse({ success: true, paperId, isNew });
        break;
      }

      case 'removePaperFromProject': {
        await ProjectStorage.removePaperFromProject(params.projectId, params.paperId);
        sendResponse({ success: true });
        break;
      }

      case 'deleteProject': {
        await ProjectStorage.deleteProject(params.projectId);
        sendResponse({ success: true });
        break;
      }
...
/**
 * Handle Zotero operations (optional)
 */
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
